...
class Replica:
    def replica_file(self, rse, scope, name, data_file, sidecar_file, pfn=None):
        with open(sidecar_file, 'r') as f:
            sidecar = f.readlines()

        sidecar_json = ''.join(sidecar)
    
        self.replica(rse, scope, data_file, sidecar_json, pfn=None):

    def replica_json(self, rse, scope, name, data_file, sidecar, pfn=None):
